...
if "SESSION_ID" not in st.session_state:
    # DB에서 마지막 세션 ID 가져오기
    res = supabase.table("trade_log").select("session_id").order("trade_id", desc=True).limit(1).execute()
...
if st.sidebar.button("🔄 성과 초기화 + 새 매매 시작"):
    ...
    # ✅ DB 기록 삭제
    supabase.table("trade_log").delete().eq("session_id", SESSION_ID).execute()
